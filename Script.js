...
async function syncUserAdsBalance(userId) {
    ...
    try {
        ...
        const userData = userDoc.data();
        const rubiBalance = userData.rubiBalance || 0;
        ...
        querySnapshot.forEach((doc) => {
            ...
            const expectedAmount = adData.type === 'sell' ? rubiBalance : usdtBalance / (adData.price || 1);
            if (Math.abs(adData.availableAmount - expectedAmount) > 0.01) {
                batch.update(doc.ref, { 
                    availableAmount: expectedAmount, 
                    updatedAt: new Date() 
                });
                updated = true;
            }
        ...
        if (updated) {
            await batch.commit();
            console.log(`Synced ${querySnapshot.size} active ads for ${userId}`);
        ...
        console.error(`Error syncing ads balance for ${userId}:`, error);
    }
}
...
async function loadUserProfile(attempt = 1, maxAttempts = 3) {
    ...
    try {
        ...
        if (userDoc.exists()) {
            ...
            // Sync ads only if balances have changed externally
            const adsQuery = query(collection(db, 'ads'), where('ownerId', '==', userId), where('status', '==', 'active'));
            const querySnapshot = await getDocs(adsQuery);
            let needsSync = false;
            querySnapshot.forEach(doc => {
                const adData = doc.data();
                const expectedAmount = adData.type === 'sell' ? userData.rubiBalance : userData.usdtBalance / (adData.price || 1);
                if (Math.abs(adData.availableAmount - expectedAmount) > 0.01) {
                    needsSync = true;
                }
            });
            if (needsSync) {
                await syncUserAdsBalance(userId);
            }
        } else {
        ...
        alert('Failed to load profile after multiple attempts.');
...
window.updateAvailableAmount = async function () {
    const userId = currentUser ? currentUser.uid : GUEST_UID;
    console.log(`Updating available amount for userId: ${userId}`);
    ...
    try {
        const userDoc = await getDoc(doc(db, 'users', userId));
        const userData = userDoc.data();
        const availableAmount = adType === 'sell' ? userData.rubiBalance || 0 : userData.usdtBalance || 0;
        availableAmountInput.value = availableAmount.toFixed(2);
        availableAmountInput.readOnly = true;
        availableAmountInput.placeholder = `Available: ${availableAmount.toFixed(2)} ${adType === 'sell' ? 'RUBI' : 'TZS'}`;
        console.log(`Available amount set to ${availableAmount} for ${adType}`);
    } catch (error) {
        console.error(`Error fetching user balance for ${userId}:`, error);
        availableAmountInput.value = '0.00';
        availableAmountInput.readOnly = true;
...
window.createAd = async function () {
    ...
    try {
        ...
        if (!type || !price || price <= 0 || !minAmount || minAmount <= 0 || !maxAmount || maxAmount <= 0 || !timeLimit || timeLimit < 5) {
        ...
        const userDoc = await getDoc(doc(db, 'users', userId));
        const userData = userDoc.data();
        const availableAmount = type === 'sell' ? userData.rubiBalance || 0 : userData.usdtBalance / price || 0;
        ...
        console.log(`Ad created with ID: ${adDoc.id} for ${userId}`);
        localStorage.removeItem('ads_all');
        localStorage.removeItem('ads_all_CacheTime');
    ...
    try {
        ...
        await runTransaction(db, async (transaction) => {
            ...
            if (currentOrder.type === 'buy') {
                ...
                // Update ad's availableAmount: decrement for other's ad, sync with balance for user's ad
                newAvailableAmount = adData.ownerId === userId ? newUserBalance : adData.availableAmount - rubiAmount;
                ...
                // Update ad's availableAmount: decrement for other's ad, sync with balance for user's ad
                newAvailableAmount = adData.ownerId === userId ? newUserBalance / adData.price : adData.availableAmount - rubiAmount;
        ...
        // Handle potential false permission errors
        ...
        if (error.code === 'permission-denied') {
            ...
            const orderQuery = query(
                ...
                where('createdAt', '>', new Date(Date.now() - 30000)) // Last 30 seconds
            );
            const userRef = doc(db, 'users', userId);
            const adRef = doc(db, 'ads', currentOrder.adId);
            ...
            if (!orderSnapshot.empty && userDoc.exists() && adDoc.exists()) {
                ...
                const balanceUpdated = expectedBalance <= inputAmount; // Check if balance decreased
                const adAmountCorrect = adData.ownerId === userId 
                    ? (currentOrder.type === 'buy' 
                        ? adData.availableAmount === userData.rubiBalance 
                        : adData.availableAmount === userData.usdtBalance / adData.price)
                    : (currentOrder.type === 'buy' 
                        ? adData.availableAmount <= currentOrder.ad.availableAmount - order.rubiAmount 
                        : adData.availableAmount <= currentOrder.ad.availableAmount - order.rubiAmount);
...
window.showDepositModal = function () {
    ...
    if (depositMethod) depositMethod.value = 'M-pesa (Vodafone)';
    if (depositBtn) {
...
window.showDepositRubiModal = function () {
    ...
    const depositRubiMethod = document.getElementById('deposit-rubi-method');
    ...
    if (depositRubiMethod) depositRubiMethod.value = 'M-pesa (Vodafone)';
    if (depositRubiBtn) {
...
window.processDeposit = async function () {
    ...
    try {
        ...
        console.log(`Deposit transaction of ${amount} USDT created (pending) for ${userId}`);
        alert('Deposit request submitted. Awaiting confirmation.');
        closeModal('deposit-modal');
        // Note: User balance (usdtBalance) and ads will be updated externally when transaction is confirmed
    } catch (error) {
...
window.processRubiDeposit = async function () {
    ...
    const method = document.getElementById('deposit-rubi-method')?.value;
    ...
    try {
        ...
        console.log(`Rubi deposit transaction of ${amount} RUBI created (pending) for ${userId}`);
        alert('Rubi deposit request submitted. Awaiting confirmation.');
        closeModal('deposit-rubi-modal');
        // Note: User balance (rubiBalance) and ads will be updated externally when transaction is confirmed
    } catch (error) {
...
window.processWithdrawal = async function () {
    ...
    try {
        ...
        alert('Withdrawal request submitted. Awaiting confirmation.');
...
window.processRubiWithdrawal = async function () {
    ...
    try {
        ...
        alert('Rubi withdrawal request submitted. Awaiting confirmation.');
